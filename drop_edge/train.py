...
This is for an experiment of NFP by Drop-edge.
...
The conventional approach of NFP is very slow.
Drop-edge speeds up generating NFP by dropping atomic edges except
randomly selected one edge per one atom.
Selected edges may differs to another step of generating NFP.
...
parser.add_argument('-x', '--data', type=int, default=5000,
                    help='number of data of NN train')
parser.add_argument('-y', '--datatest', type=int, default=1000,
                    help='number of data of NN test')
parser.add_argument('-g', '--gpu', action='store_true', help='GPU mode')
...
net = nn.NN(d=d, batchsize=batchsize, n_train_epoch=n_train_epoch,
            n_val_epoch=n_val_epoch, n_units=m, gpu=args.gpu)

# Setup NFP generator
_nfp = fp.nfp(d, f, R, args.gpu)
